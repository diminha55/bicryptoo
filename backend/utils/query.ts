...
export async function updateRecord(
  ...
  try {
    ...
    try {
      const { image, avatar, icon } = updateData;
      if (existingRecord && existingRecord.image && !image) {
        await deleteFile(existingRecord.image);
      }
      if (existingRecord && existingRecord.avatar && !avatar) {
        await deleteFile(existingRecord.avatar);
      }
      if (existingRecord && existingRecord.icon && !icon) {
        await deleteFile(existingRecord.icon);
      }
    } catch (error) {
