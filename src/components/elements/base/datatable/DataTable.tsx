...
const DataTableBase = ({
  ...
  const [dynamicColumnWidth, setDynamicColumnWidth] = useState<string | number>(
    "auto"
  );
  const measureTableWidth = useCallback(() => {
    if (tableRef.current) {
      ...
      const dynamicColumnWidth =
        tableWidth && dynamicColumns > 0
          ? (Number(tableWidth) - totalFixedWidth) / dynamicColumns
          : "auto";
      setDynamicColumnWidth(dynamicColumnWidth);
  ...
  return (
    <div id="datatable" className="relative h-full">
      <AnimatePresence>
          ...
          <motion.div
            ...
            className={
              paginationLocation === "floating"
                ...
                : `absolute bottom-0 w-full flex gap-4 items-start ${
                    shape !== "straight" && "pt-5"
                  }`
            }
          >
