...
export class MatchingEngine {
  ...
  private async initializeOrders() {
    try {
      ...
      openOrders.forEach((order) => {
        ...
        // Ensure the userid and id have a buffer property
        if (!order.userId?.buffer || !order.id?.buffer) {
          console.error("Invalid Uuid in order:", order);
          return;
        }
        ...
        const normalizedOrder = {
          ...
          createdAt, // Use validated Date object
          updatedAt, // Use validated Date object
          userId: uuidToString(order.userId), // Convert Uuid to string
          id: uuidToString(order.id), // Convert Uuid to string
        };
  ...
  private async initializeYesterdayCandles() {
    try {
      ...
      Object.keys(yesterdayCandles).forEach((symbol) => {
        ...
        if (candles.length > 0) {
          this.yesterdayCandle[symbol] = candles[0]; // Assuming the latest candle for the previous day
        }
      });
    } catch (error) {
  ...
  private generateCandleQueries(
    ...
    if (shouldCreateNewCandle) {
      ...
      existingLastCandle.updatedAt = now; // Add this line to update timestamp
  ...
  async broadcastUpdates(
    ...
    // Create promises for Order Updates
