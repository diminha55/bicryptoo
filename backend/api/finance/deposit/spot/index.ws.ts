...
export default async (data: Handler, message) => {
  ...
  const transaction = await models.transaction.findOne({
    where: { referenceId: trx, userId: user.id },
  });
...
export async function verifyTransaction(
  ...
  try {
    if (exchange.has["fetchDeposits"]) {
      deposits = await exchange.fetchDeposits(transaction.wallet?.currency);
  ...
  const deposit = deposits.find((d) => d.txid === transaction.referenceId);
  ...
  if (
    ["kucoin", "binance", "okx"].includes(provider) &&
    transaction.wallet?.currency !== deposit.currency
  ) {
    sendMessage(payload, {
      status: 400,
  ...
  if (
    ...
    settings.get("depositExpiration").value === "true"
  ) {
  ...
  const updatedTransaction = await updateTransaction(transaction.id, {
    status: "COMPLETED",
    description: `Deposit of ${amount} ${transaction.wallet?.currency} to wallet`,
    amount: amount,
    fee: fee,
  });
  ...
  const updatedWallet = (await updateSpotWalletBalance(
    userId,
    updatedTransaction.wallet?.currency,
    amount,
    fee,
    "DEPOSIT"
  )) as walletAttributes;
  ...
  try {
    ...
    await handleNotification({
      ...
      message: `Your deposit of ${amount} ${updatedTransaction.wallet?.currency} has been confirmed.`,
    });
  } catch (error) {
