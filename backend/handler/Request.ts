...
export class Request {
  ...
  constructor(private res: HttpResponse, private req: HttpRequest) {
    this.url = req.getUrl();
  ...
  private async readRequestBody(): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      // Explicitly define bodyData as an array of strings

      this.res.onData((ab: ArrayBuffer, isLast: boolean) => {
        // Convert the ArrayBuffer to a string and add it to bodyData
        const chunk = Buffer.from(ab).toString();
        bodyData.push(chunk);

        if (isLast) {
          // When the last chunk is received, join the bodyData array into a single string
  ...
  private processBodyContent(contentType: string, bodyContent: string): any {
    // Check if the content type is JSON and the body is not empty
    if (contentType.includes("application/json") && bodyContent.trim() !== "") {
      try {
    ...
    // Return as raw text or an empty object for other content types or if the body is empty
  ...
  // update tokens ({ accessToken, csrfToken, sessionId })
