...
const EditKycTemplate = () => {
  ...
  useEffect(() => {
    ...
    const fetchTemplate = async () => {
      ...
      if (!error && data) {
        ...
        const customOptionsArray = Object.keys(customOptions || {}).map(
          (key) => ({
            title: key,
            ...customOptions[key],
          })
  ...
  const renderFieldSettings = (field: string) => (
    <div
      ...
      className={`p-5 flex xs:flex-col sm:flex-row justify-between items-start gap-5 text-muted-800 dark:text-muted-200 ${
        !["ssn", "documentIdCard"].includes(field)
  ...
  return (
    <Layout title={t("Edit KYC Template")} color="muted">
      <div className="flex justify-between items-center w-full mb-5">
        <h1 className="text-xl">{t("Edit KYC Template")}</h1>
...
export default EditKycTemplate;
