...
export default async (data: Handler) => {
  ...
  if (!exchange) throw createError(500, "Failed to start exchange");
  ...
  try {
    ...
    if (!orderData || !orderData.id) throw createError(404, "Order not found");
    ...
    await sequelize
      .transaction(async (transaction) => {
        await models.wallet.update(
          { balance: balanceUpdate }, // Assuming balanceUpdate is the new balance
