...
export async function createKyc(
  ...
  const existingKycData = existingKyc.data ? JSON.parse(existingKyc.data) : {};
...
function deepMerge(obj1: any, obj2: any): any {
  ...
  Object.keys(obj2).forEach((key) => {
    if (obj2[key] === null && key === "documents") {
      output[key] = obj1[key]; // Keep the existing documents if new ones are null
