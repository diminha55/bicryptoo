...
export function addTradeToOrder(order: Order, trade: TradeDetail) {
  ...
  if (order.trades) {
    try {
      // Check if the trades are already stringified and valid JSON
      if (typeof order.trades === "string") {
        ...
        // If the trades are not in the correct format, log an error and reset to an empty array
  ...
  // Merge the new trade into the existing trades
  const mergedTrades = [...trades, trade].sort(
    (a, b) => a.timestamp - b.timestamp
  );

  // Update the order's trades using BigIntReplacer
...
export function validateOrder(order: Order): boolean {
  if (
    ...
    !order.feeCurrency ||
    !order.status ||
    !(order.createdAt instanceof Date) || // Validate if it's a Date object
    !(order.updatedAt instanceof Date) // Validate if it's a Date object
  ) {
